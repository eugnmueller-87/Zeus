...
def run_pipeline(signals: list[RawSignal], label: str) -> None:
    ...
    for raw in signals:
        ...
        macro = artemis.analyze(filtered)
        if macro.suppress:
            print(f"  → Artemis: SUPPRESS — {macro.suppress_reason}\n")
            continue
        print(f"  → Artemis: PASS (regime={macro.regime} VIX={macro.vix:.1f})")

        sized = pythia.size(filtered, macro)
        if sized.skip:
            print(f"  → Pythia: SKIP — {sized.skip_reason}\n")
            continue
        print(f"  → Pythia: size={sized.position_size_pct*100:.2f}% confidence={sized.confidence:.2f}")

        result = ares.place(sized)
        print(f"  → Ares: {result.side} {result.symbol} @ {result.fill_price} | id={result.order_id}")
        pythia.record_trade(sized, result)
        print()
...
if __name__ == "__main__":
    ...
    if args.company:
        icarus = IcarusAgent(api_key=os.getenv("HERMES_API_KEY", ""))
